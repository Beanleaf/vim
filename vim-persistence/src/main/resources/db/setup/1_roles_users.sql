...
/* Create dba and application user */
DO
$$
    BEGIN
        IF NOT EXISTS(
...
$$
    BEGIN
        IF NOT EXISTS(
                SELECT *
                FROM pg_catalog.pg_roles
                WHERE rolname = 'vim_ro'
            )
        THEN
            EXECUTE 'CREATE ROLE vim_ro LOGIN PASSWORD ''vim_ro'' VALID UNTIL ''infinity'';';
        END IF;
    END
$$;
...
$$
    BEGIN
        ...
        EXECUTE 'GRANT ALL ON DATABASE ' || current_database() || ' TO vim_dba;';
